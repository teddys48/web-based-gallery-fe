...
- **Sorting & Timeline Filtering**: Interactive sort controls supporting **Newest**, **Oldest**, **Name (A-Z)**, and **Name (Z-A)** sorting, both globally and on individual timeline date cards (next to item count pills), alongside dynamic **Timeline Bucket filtering (Year/Month)** for folder media, seamlessly updating grid layouts and Lightbox navigation order.
